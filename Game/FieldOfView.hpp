/*
** EPITECH PROJECT, 2021
** G-JAM-001-STG-0-1-jam-louis.kleiver
** File description:
** FieldOfView
*/

#ifndef FIELDOFVIEW_HPP_
#define FIELDOFVIEW_HPP_

#include "ECS.hpp"
#include "AScene.hpp"
#include "AEntity.hpp"

#include "Transform2DComponent.hpp"
#include "ShapeComponent.hpp"
#include "LayerComponent.hpp"
#include "TextureComponent.hpp"

#include "PhysicsComponent.hpp"
#include "CollisionComponent.hpp"

class FieldOfView  : virtual public gameEngine::AEntity {
    public:
        FieldOfView(gameEngine::ECS *ecs) : AEntity(ecs, "FieldOfView")
        {
        }

        ~FieldOfView() = default;

        void init(gameEngine::ECS *ecs)
        {
            init(ecs, gameEngine::Vector2<float>(0, 0));
        }

        void init(gameEngine::ECS *ecs, const gameEngine::Vector2<float> &pos, /*const gameEngine::Rectangle<float> &bounces,*/ const gameEngine::Vector2<float> &scale = {1.6, 1.6})
        {
            gameEngine::Transform2DComponent transform;
            transform._transform.setScale(scale);
            transform._transform.setPosition(pos);

            gameEngine::ShapeComponent shape;
            shape._type = gameEngine::ShapeType::RectangleShape;
            shape._textureCenterRatio = {0.5, 0};
            shape._isComplex = true;
            shape._complexBounce = {
                gameEngine::Vector2<float>(24, 161),
                gameEngine::Vector2<float>(99, 6.5),
                gameEngine::Vector2<float>(101, 6.7),
                gameEngine::Vector2<float>(176, 161)
            };
            //shape._rectangle = bounces;
            shape._baseOnTexture = true;


            gameEngine::LayerComponent layerCmp;
            layerCmp.layer = 25;

            gameEngine::TextureComponent texture;
            texture._type = gameEngine::TextureType::FileTexture;
            texture._index = ecs->getScene()->getGraphic()->loadTexture("assets/Generic/FieldOfView.png");
            texture._rect = {0, 0, 200, 200};
            texture._color = {255, 255, 255, 100};

            gameEngine::PhysicsComponent physics;
            gameEngine::CollisionComponent collision;

            ecs->addComponents(this, transform, shape, layerCmp, texture, physics, collision);
        }


    protected:
    private:
};

#endif /* !FIELDOFVIEW_HPP_ */
